...
- 新增 `sinan-captcha` 文档源，接入本地 `/Users/uroborus/AiProject/sinan-captcha/docs` 与远程 `https://github.com/uroborus2s/sinan-captcha.git@main` 双模式配置，并补齐 git submodule 登记
- 将 `crawler4j` 文档源的远端同步分支从 `feature/task-plugin-system` 切换为 `main`，并同步更新根仓 submodule 跟踪分支
- 验证 `crawler4j@main` 时发现 `docs/index.md` front matter 含反引号标题，当前会触发 YAML 解析错误，后续需在源仓修复
