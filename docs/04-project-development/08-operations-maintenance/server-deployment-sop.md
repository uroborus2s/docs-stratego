...
- `deploy/.env` 已配置 `DB_USER`、`DB_PASSWORD`、`DB_NAME`
- Casdoor `driverName=postgres`
- Casdoor `dataSourceName=${DB_DSN}`
- `oidc_issuer_url`
- `redirect_url`
- `client_id`
- `client_secret`
...
然后触发一次 GitHub Actions 发布，让 Runner 把第一版 `site/` 和 `private_locations.conf` 安装到服务器。证书、GitHub App 和正式 HTTPS 配置的完整步骤，统一按 [安装说明](../../02-user-guide/installation.md) 执行。
