...
- 公开页面和私有页面继续共用同一套导航，页面上不额外显示“锁定”标记
- 匿名用户点击私有页面时，站点会同步拉起一个独立的小窗登录框
- 小窗先访问 `/oauth2/sign_in`，再进入 Casdoor 登录页
- 登录成功后，小窗会通过同源回调页通知主页面并自动关闭
- 已登录用户再次点击私有页面时，会优先走站内即时导航，不再额外做整页跳转
- 如果用户关闭登录小窗，当前公开页面保持不动，可继续浏览公开文档
- 如果用户点回主页面，小窗会自动关闭，不会仅仅退到后台
- 用户如果直接输入私有页面 URL，仍会由 `Nginx` 按路径规则触发登录链路
- `Nginx` 会通过 `auth_request` 转发到 `oauth2-proxy`
- Casdoor 支持两种登录方式：
  - 本地用户名密码
  - GitHub 登录
- 登录成功后，用户会被带回原始目标页面
...
- 如果一打开站点首页就直接跳到 `/oauth2/sign_in`、`/oauth2/start` 或 `/login`，通常不是页面权限规则问题，而是宿主机 `Nginx` 把根路径也误加了整站鉴权
